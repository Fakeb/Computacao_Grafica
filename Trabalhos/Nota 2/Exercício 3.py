...
def inicializaOpenGL():
    ...
    Window = glfw.create_window(WIDTH, HEIGHT, "Exemplo 1 - renderização de um triângulo", None, None)
...
def inicalizaTriangulo():
    ...
    pvbo = glGenBuffers(1)
    #Damos um bind no VBO criado acima para avisar o OpenGL que estamos trabalhando com ele a partir desse momento
...
def inicializaShaders():
	...
	# - a primeira linha especifica a versão da GLSL (OpenGL Shader Language) que estamos utilizando, no caso, 4.0.0
	...
	# valor de entrada (vp)
	# - a saída gl_Position deve ser um dado do tipo vec4 (4 valores x,y,z,w), por isso adicionamos 1.0 no final
    ...
    vertex_shader = """
        ...
        out vec3 posicao_fragmento; //posicao passada para o fragment shader
        void main () {
            posicao_fragmento = vertex_posicao;
	...
	# - o Fragment Shader é responsável por determinar a cor de cada fragmento/pixel da superfície do objeto
    ...
    fragment_shader = """
        #version 400
        in vec3 posicao_fragmento; //posicao do fragmento INTERPOLADA recebida do vertex shader
		out vec4 frag_colour; //sai do fragment shader e vai para a tela do computador (RGBA)
		void main () {
            float distCima, distDireita, distEsquerda;
            vec3 cima, direita, esquerda;
            cima = vec3(0.0, 0.5, 0.0);
            direita = vec3(0.5, -0.5, 0.0);
            esquerda = vec3(-0.5, -0.5, 0.0);

            distCima = distance(posicao_fragmento, cima); //calcula a distancia entre o fragmento e o vértice de cima
            distDireita = distance(posicao_fragmento, direita); //calcula a distancia entre o fragmento e o vértice da direita
            distEsquerda = distance(posicao_fragmento, esquerda); //calcula a distancia entre o fragmento e o vértice da esquerda

		    if(distCima < distDireita && distCima < distEsquerda){ //fragmento mais próximo do vértice de cima
                frag_colour = vec4 (1.0, 0.0, 0.0, 1.0); //vermelho
            }
            else if(distDireita < distCima && distDireita < distEsquerda){ //fragmento mais próximo do vértice da direita
                frag_colour = vec4 (0.0, 1.0, 0.0, 1.0); //verde
            }
            else if(distEsquerda < distCima && distEsquerda < distDireita){ //fragmento mais próximo do vértice da esquerda
...
def inicializaRenderizacao():
    ...
    while not glfw.window_should_close(Window):
        # Limpamos o buffer de cores da tela e especificando a cor de fundo
...
# Função principal
def main():
    inicializaOpenGL() #inicializa o OpenGL
    inicalizaTriangulo() #modela o triangulo
    inicializaShaders() #especcifica os shaders
    inicializaRenderizacao() #inicializa a renderização dos objetos na tela
