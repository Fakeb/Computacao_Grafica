...
def inicializaObjetos():
    ...
    points = [
		0.0, 0.95, 0.0, #cima
		0.95, -0.5, 0.0, #direita
		-0.95, -0.5, 0.0 #/esquerda
	]
...
def inicializaShaders():
    ...
    vertex_shader = """
        ...
        out vec3 posicao_fragmento;
        void main () {
            posicao_fragmento = vertex_posicao;
            gl_Position = vec4 (vertex_posicao.x*2.0,vertex_posicao.y*0.5,vertex_posicao.z, 1.0);
        }
    """
    ...
    fragment_shader = """
        #version 400
        in vec3 posicao_fragmento;
		out vec4 frag_colour;
		void main () {
            float distanciaCima, distanciaEsquerda, distanciaDireita;
            vec3 cima, esquerda, direita;
            cima = vec3(0.0, 0.95, 0.0);
            esquerda = vec3(-0.95, -0.5, 0.0);
            direita = vec3(0.95, -0.5, 0.0);

            distanciaCima =  distance(posicao_fragmento, cima);
            distanciaEsquerda =  distance(posicao_fragmento, esquerda);
            distanciaDireita =  distance(posicao_fragmento, direita);

            if(distanciaCima< distanciaDireita && distanciaCima < distanciaEsquerda){
                frag_colour = vec4 (1.0, 0.0, 0.0, 1.0);
            }
            else if(distanciaDireita < distanciaCima && distanciaDireita < distanciaEsquerda){
                frag_colour = vec4 (0.0, 1.0, 0.0, 1.0);
            }
            else if(distanciaEsquerda < distanciaCima && distanciaEsquerda < distanciaDireita){
